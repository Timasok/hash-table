...
Также необходимо размер таблицы округлить до простого числа, чтобы при вычислении индекса ключа `i` не возникло дополнительных коллизий. Например, если выполняются условия:
...
| Base version | 120  |   1   |   1   |
|     -O3      | 25.4 |  4.72 |  4.72 |
|  *Без strncpy* | 25.0 |  4.80 |  1.01 |
|  *с -no-pie*   | 21.2 |  5.66 |  1.18 |
|   Asm_hash   | 25.6 |  4.69 |  0.83 |

Рассмотрим также две побочных оптимизации, возникших по ходу ассемблерной.

Во-первых удалось уменьшить число вызовов `__strncpy_avx2`. Вместе с выравниванием длин, мы догадались выделять динамическую память сразу для всего набора слов. Это можно оставить при дальнейшей работе.

Во-вторых мы заметили, что с флагом `-no-pie` происходит незначительное ускорение, однако не стоит забывать про то, что несёт за собой его использование. Исполняемый файл перестаёт быть `PIE` (англ. `Poisition Independent Executable` - исполняемый файл, c полной относительной адресацией). Это делает программу уязвимой для хакеров, поэтому не будем дальше его применять. 

Что касается основной оптимизации, то она не оправдала наши ожидания, поэтому оставим пока сишный вариант хэша.

Также для улучшения статистики, после удаления `__strncpy_avx2`, поднимем число запусков до `10000`. Вот результат:

![cmp_number_of_tests](./callgrind/cmp_number_of_tests.png)

**Шаг 3. Замена хэш функции**

Хэш-функция настолько часто вызывается, что даже небольшое ускорение значительно ускорит работу функции. Суть этой оптимизации - доказать, что выравнивание адресов под `32` бита в купе с `SIMD`- инструкциями творят чудеса. 
...
|        -O3       | 25.4 |  4.72 |  4.72 |
|    *Без strncpy*   | 25.0 |  4.80 |  1.01 |
|  Замена на crc32 | 20.6 |  5.82 |  1.21 |



Дальнейшие оптимизации ещё планируются (пока на этом остановимся)

<!-- **Шаг 4. Замена хэш функции**

Так как наше ограничение задачи(использование слов до 8 символов) не позволяет в полной мере воспользоваться 256-битными командами, но это интересный приём оптимизации, то позволим себе немного изменить формулировку задачи. Будем оптимизировать функцию `findInTable`. -->


<!-- На этом этапе оптимизации в исходном коде заканчиваются -->
