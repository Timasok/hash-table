...
4. При каждом тесте будем:
    + Если номер теста чётный - искать слово из Толкиена, если нечётный, то из Шекспира. 
...
**Выбор флага оптимизации**

Определим флаг оптимизации `-O`, с максимальным ускорением. 
...
`-O3` показал наибольшее ускорение, значит запуск с этим флагом следует считать как начальную величину.
